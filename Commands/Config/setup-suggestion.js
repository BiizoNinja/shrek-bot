...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (data) return message.channel.send('<:wrong:856162786319925270> You already have sey-up tickets in this server!')
        ...
        if (!data) {
            ...
            function startMessageCollectors(client, message, args, filter) {
                ...
                const channelColl = new Discord.MessageCollector(
                    message.channel,
                    filter, {
                        max: 10
                    }
                ...
                channelColl.on('collect', async (msg) => {
                    ...
                    if (!channel) {
                        msg.channel.send('<:wrong:856162786319925270> You need to mention a channel! Set-up failed')
                        channelColl.stop();
                    } else {
                    ...
                    roleColl.on('collect', async (msg) => {
                        ...
                        if (!role) {
                            msg.channel.send('<:wrong:856162786319925270> You need to mention a role! Set-up failed')
                            roleColl.stop();
                        } else {
                            message.channel.send(new Discord.MessageEmbed()
