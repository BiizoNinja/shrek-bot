...
module.exports = {
  ...
  run: async (client, message, args) => {
    if (!message.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR)) return message.channel.send(`${emojis.wrong} You need the \`ADMINISTRATOR\` permission to use this!`)
    ...
    if (!suggestData) return message.channel.send(`${emojis.wrong} This server hasn\'t set-up suggestions!\nRun \`.setup-suggestion\` to setup.`)
    if (!setting) {
      ...
      if (setting == 'channel') {
        ...
        if (!channel) return message.channel.send({content: `${emojis.wrong} Please mention a channel.`})
        ...
        message.channel.send({content: `${emojis.success} Successfully updated the suggestion channel to ${channel}\nPeople can use the \`.suggest\` command and the suggestion will show up there.`})
      }

      if (setting == 'role') {
        const role = message.mentions.roles.last() || message.guild.roles.cache.get(args[2])
        if (!role) return message.channel.send({content: `${emojis.wrong} Please mention a channel.`})
        ...
        message.channel.send({content: `${emojis.success} Successfully updated the suggestion channel to ${role.name}**\nPeople with the role can accept or deny suggestions.`})
      }
    }
