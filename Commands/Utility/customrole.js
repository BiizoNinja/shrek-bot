...
const crUser = require('../../models/cr-userSchema')
  ...
  if(!settings) return message.channel.send(`<:wrong:856162786319925270> You need to specify an option! \`.customrole [create | settings | cache | help]\``);
  ...
  if(settings.toLowerCase() == 'settings') {
      ...
      if(setting == 'SetPos' || setting == 'SetPosition') {
          ...
          const MaxPosValue = parseInt(args[2])
            if(!MaxPosValue) return message.channel.send('<:wrong:856162786319925270> You need to provide a value to change!')
            if(isNaN(MaxPosValue)) return message.channel.send('<:wrong:856162786319925270> You need to provide a value that is a NUMBER for your role.')
            ...
            message.channel.send(`<:greenTick:854228019312066571> Updated the role position to "${MaxPosValue}"`);
    
      };

      if(setting == 'AllowedRole' || setting == 'Role' || setting == 'UsableRole' || setting == 'UseRole') {
        const role =  message.guild.roles.cache.get(args[2])
        if(!role) return message.channel.send('<:wrong:856162786319925270> Please provide a valid role id!')
      ...
      message.channel.send(`<:greenTick:854228019312066571> Updated the allowed role to "**${role.name}**"`)
      };
  }; 
  ...
  if(settings.toLowerCase() == 'create') {
    ...
    if(!crUserData) {
      ...
      if(!name) return message.channel.send('<:wrong:856162786319925270> Please provide a name for your role!')
  
      const color = args[1]
      if(!color) return message.channel.send('<:wrong:856162786319925270> Please provide a color for your role!')
  
      const reg = new RegExp("^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$")
      const match = color.match(reg)
      if(!match) return message.channel.send('<:wrong:856162786319925270> Please provide a valid hex code! Make sure to use `#` at the beginning')
    ...
    if(crUserData) {
      message.channel.send('<:wrong:856162786319925270> You already have a customrole! Run `.customrole delete` to delete it!')
    };
 };
 ...
 if(settings.toLowerCase() == 'delete') {
  ...
  message.channel.send('<:greenTick:854228019312066571> I have deleted your customrole!')
 }
 ...
 if(settings.toLowerCase() == 'help') {
   ...
   const embed1 = new Discord.MessageEmbed()
     .setAuthor(`Help - Customrole`, client.user.displayAvatarURL({dynamic: true}))
     .setDescription('Custom Roles are a way to reward your server members! You can set up custom roles in your server using ShrekBot! If you have more questions feel free to ask in the [Support Server](https://discord.gg/V9DHGNtuUe)')
     .addFields(
       {name: 'Caching', value: 'Before you can use ANY of the commands you need to cache your server! (add your server to our database) Administrators can do this by running `.customrole cache`.'},
       {name: 'View Settings', value: 'Admins can view the server settings by running the command `.customrole settings`'}
     )
     .setColor('#A6FE00')
   ...
   const embed2 = new Discord.MessageEmbed()
       ...
       .setDescription('Custom Roles are a way to reward your server members! You can set up custom roles in your server using ShrekBot! If you have more questions feel free to ask in the [Support Server](https://discord.gg/V9DHGNtuUe)')
       .addFields(
         {name: 'Max Position', value: 'This is the position where the role will appear, role positions go from bottom to top in the role list! To set this you can do `The role you want to go above position + 2` For example: Server Booster role position is 12 so the command will be `.cr settings MaxPos 14`(12 + 2)\nAdmins can set position by running `.customrole settings SetPos <Position>`'},
         {name: 'Allowed Role', value: 'This determins weather a user should have a specific role to create custom roles!\nAdmins can change bu using the command `.customrole settings AllowedRole <RoleID>`'},
         {name: 'Creating Roles', value: 'People with the allowed role can create their roles by running `.customrole create <HexColor> <RoleName>` Make sure to use `#` at the beginning for the hex code.'}
       )
       .setColor('#A6FE00')
